...
const giverFor = row => row.model === 'missions' || row.model === 'quests' ? 'Alma' : '';
...
async function main(){
  ...
  for(const section of missionsHtml.split(/<h4[^>]*>/i).slice(1)){
    ...
    for(const m of body.matchAll(/href="\/data\/missions\/([^"]+)">([\s\S]*?)<\/a>/gi)){
      const row=byId.get(`missions-${m[1]}`);if(row){row.boardGroup=group;row.questType='story';row.mainMonsters=[];row.giver=giverFor(row);}
    }
  }
