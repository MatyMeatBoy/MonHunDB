#!/usr/bin/env python3
"""Check exact matches - with UTF-16 encoding"""

# Downloaded files (UTF-16!)
with open('downloaded.txt', 'r', encoding='utf-16') as f:
...
def slugify(name):
    return name.lower().replace(" ", "-").replace("'", "").replace("+", "").replace(".", "").replace("é", "e").replace("’", "")
...
for s in implied_sets:
    ...
    if slug in downloaded:
        continue
    # Try alternate: remove 's after apostrophe
    alt = s.replace("'s", "").replace("'", "")
    alt_slug = slugify(alt)
    if alt_slug in downloaded:
        print(f"  MATCH (alt): {s} -> {alt_slug}")
    else:
        missing.append((s, slug, alt_slug))

print(f"\n=== MISSING ({len(missing)}) ===")
for s, slug, alt in missing:
    print(f"  {s}: expected '{slug}' or '{alt}'")
