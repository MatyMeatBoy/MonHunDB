...
const MHFU_ITEM_ICON_FALLBACKS = {
  ...
  // Source abbreviations and event-only names do not have their own sprite
  // record.  Use the matching local in-game item sprite, rather than leaving
  // a blank material slot in armor, weapon and quest requirements.
  "Amezari Carapace": "gt-lobstershell.png",
  "Amezari Shell": "gt-lobstershell.png",
  "AncientFlatstone": "flat-ruststone.png",
  "Book of Combos 1": "anc-drgn-treasre.png",
  "Book of Combos 5": "anc-drgn-treasre.png",
  "Ceanataur Pincer": "ceanataur-claw.png",
  "Congalala Pelt": "conga-pelt.png",
  "Congalala Pelt+": "hrdcongalalaplt.png",
  "Dengeki G Ticket": "hard-ticket.png",
  "Dengeki Maoh Tkt": "hard-ticket.png",
  "Dengeki Ticket": "normal-ticket.png",
  "Famitsu Invoice": "hard-ticket.png",
  "Famitsu PT Tckt": "hard-ticket.png",
  "Famitsu Ticket": "normal-ticket.png",
  "FamitsuCustomTkt": "hard-ticket.png",
  "Fire Drgn Scl+": "rathalos-scale.png",
  "Garuga Carapace": "hvy-garuga-shell.png",
  "Grn Plsioth Fin+": "plesioth-scale.png",
  "HardFatalisHorn": "fatalis-horn.png",
  "Hunter Soul Tkt": "hunter-s-ticket.png",
  "Hvy Rajang Fang": "rajang-fang.png",
  "LapisLazuliJewel": "akito-jewel.png",
  "LeatherNargaTail": "nargacuga-tail.png",
  "LthrSlvrRthlsTal": "slvr-rthlos-tail.png",
  "LunastraCarapace": "lunastra-shell.png",
  "Magazine Ticket": "hard-ticket.png",
  "Pirate J Tckt G": "hard-ticket.png",
  "Pirate J Ticket": "normal-ticket.png",
  "Plus Class Tcket": "hard-ticket.png",
  "Pur Gypceros Wng": "hrdpugypceroswng.png",
  "Rejuvenated Beak": "splendid-beak.png",
  "ThckWhtFatlisScl": "wht-fatalis-scl.png",
  "Top-GrdRedHorn": "blood-red-horn.png",
  "VespoidQn'sCrown": "vespoid-carapace.png",
  "Vspoid Razorwing": "hornet-razorwing.png",
  "YamaTsukaFillet": "yamatsukamifluid.png",
};
...
function selectMonster(name, opts = {}) {
  ...
  if (opts.render) {
    ...
    if (!monster) return;
    hideViews(homeViewEl, decorationsViewEl, weaponsViewEl, armorViewEl, materialsViewEl, skillsViewEl, questsViewEl);
...
function showDecorationsView() {
  hideViews(detailEl, homeViewEl, weaponsViewEl, armorViewEl, materialsViewEl, skillsViewEl, questsViewEl);
...
function showWeaponsView() {
  hideViews(detailEl, homeViewEl, decorationsViewEl, armorViewEl, materialsViewEl, skillsViewEl, questsViewEl);
...
function showArmorView() {
  hideViews(detailEl, homeViewEl, decorationsViewEl, weaponsViewEl, materialsViewEl, skillsViewEl, questsViewEl);
...
function bootArmor() {
  ...
  if (setName) {
    // showArmorSetDetail() itself resolves setName against an explicit
    // armorSets entry OR an implied/partial name-prefix group and no-ops
    // if neither matches -- the old `armorSets.some(...)` gate here broke
    // deep-linking to implied groups whenever armor_sets.json doesn't
    // exist at all (MHFU has no explicit sets, only implied ones).
    showArmorView();
...
function showMaterialsView() {
  hideViews(detailEl, homeViewEl, decorationsViewEl, weaponsViewEl, armorViewEl, skillsViewEl, questsViewEl);
...
function showSkillsView() {
  hideViews(detailEl, homeViewEl, decorationsViewEl, weaponsViewEl, armorViewEl, materialsViewEl, questsViewEl);
...
const NEWS = [
  ...
  },
  {
    ...
    imageHtml: () => `<img src="data/images/monsters/MH1-Rathalos.webp" alt="" loading="lazy" onerror="this.remove()">`,
  },
];
...
// ---------- Quests (MHFU-only, no Rise/Wilds equivalent) ----------

function navQuest(id) {
  const u = new URL("quests", location.href);
...
function renderQuestsIndex(query) {
  ...
  questsIndexEl.querySelectorAll(".decoration-card").forEach(btn => {
    btn.addEventListener("click", () => navQuest(btn.dataset.id));
  });
}
...
function bootQuests() {
  ...
  showQuestsView();
  if (questId && quests.some(x => x.id === questId)) showQuestDetail(questId);
}
